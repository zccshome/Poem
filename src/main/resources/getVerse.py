...
class VerseClass:
	def __init__(self, title, subtitle, author, xu, content1, content2):
		self.title = title
	...
	def __str__(self):
		return self.title + ' ' + self.subtitle + ' ' + self.author + ' ' + self.xu + ' ' + self.content1 + ' ' + self.content2
...
def getVerse():
	for i in range(1, 1486):
		...
		for verse in verses_result:
			...
			xu = verse_result.group(3)
			
			content1 = verse_result.group(4)
			#print(content1)
			content2 = verse_result.group(5)
			#print(content2)
			
			verse = VerseClass(title, subtitle, author, xu, content1, content2)
			preservePage(verse)
...
def preservePage(verseClass):
	try:
		...
		sys.exit()
	cursor = conn.cursor()
	sql = "insert into verse(title, subtitle, author, xu, content1, content2) values ('%s', '%s', '%s', '%s', '%s', '%s')" % (verseClass.title, verseClass.subtitle, verseClass.author, verseClass.xu, verseClass.content1, verseClass.content2)
	try:
		cursor.execute(sql)
	except Exception, e:
		print e
	conn.commit()
	'''
	sql = "delete from verse where title = '%s' and subtitle = '%s' and author = '%s' and content1 = '%s'" % (verseClass.title, verseClass.subtitle, verseClass.author, verseClass.xu)
	try:
		cursor.execute(sql)
	except Exception, e:
		print e
